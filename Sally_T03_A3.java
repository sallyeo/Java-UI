...
class Qna extends JFrame
{
	...
	private Participants [] pArray = {s1, s2, s3, s4};
	ArrayList <Participants> alist = new ArrayList <Participants> ();
	
	private static void arrayToList (Participants [] pArray, ArrayList <Participants> alist)
	{
		for (int i = 0; i<pArray.length; i++)
		{
			Participants p = pArray[i];
			alist.add(p);
		}
	}
	...
	public Qna ()
	{
		...
		//Jbutton.addactionlistener(e -> {whatever you have in the actionperformed method})
		
		// Register the events to each student button!!!!!!!!!!
		arrayToList (pArray, alist);
		
		int k = 0;
		for (Participants p : alist)
		{
			sjbArray[k].addActionListener (new SButtonEvent(p));
			++k;
		}
	...
	private class SButtonEvent implements ActionListener
	{
		...
		public SButtonEvent (Participants participant)
		{
